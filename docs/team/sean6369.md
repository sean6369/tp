...
**Highlights**: 
1. Validation methods: `validatePriority()`, `validateSortField()`, `validateSortOrder()`, `validateFilterType()`, `validateAndParseDate()`, `validateFilterCommand()`, `validateSortCommand()`
2. `parseIndexOrNull()` (task index: format/range checks, 0-based conversion); `validateProjectIndex()` (project index: format/range/missing cases)
3. `priorityToInt()` helper; `ValidationConstants` with priorities, filter types, sort fields/orders, keywords
4. Integrated across `FilterCommand`, `SortCommand`, `ExportCommand`, `UpdateCommand`, interactive prompts
...
**Highlights**: 
1. `InvalidCommandSyntaxException` replaces `InvalidArgumentException` for syntax errors; new exceptions for domain-specific validation
2. `ExtraArgumentException` handles unexpected parameters; context-aware constructors (`InvalidDateException` auto-generates format; `InvalidIndexFormatException` includes task/project type)
3. Enhanced `IndexOutOfRangeException` with context-aware constructors (task vs project indices); integrated across parsing, validation, export flow
