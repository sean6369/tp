...
- **Export infrastructure and view-state design**
  ...
  - **Highlights**: 
    - `TaskWithProject` associates tasks with project names (format: `"ProjectName: [X] Task Description"`); `TaskCollector` with `getAllTasksWithProjects()` and `getTasksFromProject()`
    - `TaskExporter` handles error scenarios with actionable messages (`"'<filename>': <description>"`); `isValidFilename()` validates illegal chars (`< > : " / \ | ? *`) and enforces `.txt`
    - `LegacyExportSupport` prototype with chained filter/sort and view-state tracking; core infrastructure used by `ExportCommandHandler`; view-state enables smart defaults
  - **Code**: [`TaskWithProject`](../../src/main/java/seedu/flowcli/task/TaskWithProject.java), [`TaskCollector`](../../src/main/java/seedu/flowcli/commands/utility/TaskCollector.java), [`TaskExporter`](../../src/main/java/seedu/flowcli/commands/utility/TaskExporter.java), [`ExportCommandHandler`](../../src/main/java/seedu/flowcli/commands/core/ExportCommandHandler.java), [`LegacyExportSupport`](../../src/main/java/seedu/flowcli/unused/LegacyExportSupport.java)
...
- **Command validation utilities**
  ...
  - **Highlights**: 
    - Validation methods: `validatePriority()`, `validateSortField()`, `validateSortOrder()`, `validateFilterType()`, `validateAndParseDate()`, `validateFilterCommand()`, `validateSortCommand()`
    - `parseIndexOrNull()` (task index: format/range checks, 0-based conversion); `validateProjectIndex()` (project index: format/range/missing cases)
    - `priorityToInt()` helper; `ValidationConstants` with priorities, filter types, sort fields/orders, keywords
    - Integrated across `FilterCommand`, `SortCommand`, `ExportCommand`, `UpdateCommand`, interactive prompts; date validation uses `LocalDate.parse()` with `InvalidDateException`
  - **Code**: [`CommandValidator`](../../src/main/java/seedu/flowcli/commands/validation/CommandValidator.java), [`ValidationConstants`](../../src/main/java/seedu/flowcli/commands/validation/ValidationConstants.java), [`CommandParser.parseIndexOrNull`](../../src/main/java/seedu/flowcli/parsers/CommandParser.java), [`ArgumentParser.validateProjectIndex`](../../src/main/java/seedu/flowcli/parsers/ArgumentParser.java)

- **Error handling and custom exceptions**
  - **What it does**: Implemented 5 new exception classes for granular hierarchy. `InvalidCommandSyntaxException` refactored syntax errors from `InvalidArgumentException` (structure vs values).
  ...
  - **Highlights**: 
    - `InvalidCommandSyntaxException` replaces `InvalidArgumentException` for syntax errors; new exceptions for domain-specific validation
    - `ExtraArgumentException` handles unexpected parameters; context-aware constructors (`InvalidDateException` auto-generates format; `InvalidIndexFormatException` includes task/project type)
    - Enhanced `IndexOutOfRangeException` with context-aware constructors (task vs project indices); integrated across parsing (`parseIndexOrNull`, `validateProjectIndex`), validation (`CommandValidator`), export flow
    - All extend `FlowCLIException` for consistent handling
  - **Code**: [`InvalidCommandSyntaxException`](../../src/main/java/seedu/flowcli/exceptions/InvalidCommandSyntaxException.java), [`InvalidDateException`](../../src/main/java/seedu/flowcli/exceptions/InvalidDateException.java), [`InvalidFilenameException`](../../src/main/java/seedu/flowcli/exceptions/InvalidFilenameException.java), [`InvalidIndexFormatException`](../../src/main/java/seedu/flowcli/exceptions/InvalidIndexFormatException.java), [`ExtraArgumentException`](../../src/main/java/seedu/flowcli/exceptions/ExtraArgumentException.java), [`IndexOutOfRangeException`](../../src/main/java/seedu/flowcli/exceptions/IndexOutOfRangeException.java)
