...
public class StatusCommand extends Command {
    ...
    public boolean execute(CommandContext context) throws Exception {
        ...
        if ("--all".equals(parsedArgument.getRemainingArgument())) {
        ...
        if (parsedArgument.getParsedProjectName() != null &&
                !parsedArgument.getParsedProjectName().isEmpty() &&
                !parsedArgument.getParsedProjectName().equals("--all")) {
            throw new ProjectNotFoundException(parsedArgument.getParsedProjectName());
        }

        // No arguments provided - should not happen as interactive mode handles it
        context.getUi().showAllProjectsStatus(context.getProjects());
        return true;
    }
}
